...
### 2.1 DriverOptions (编译配置选项)

`DriverOptions` 封装了编译器的所有外部输入配置。
- **输入/输出文件**: `InputFiles` (可能多个源文件), `OutputFile` (-o 指定的产物)。
- **输出模式 (DriverAction / EmitMode)**:
  - `-fsyntax-only`: 仅运行前端检查（词法、语法、语义），不进行代码生成。
  - `-dump-tokens`: 仅执行词法分析并打印 Token 流。
  - `-ast-dump` 或 `-ast-print`: 执行词法和语法分析，将 AST 打印到控制台。
  - `-S -emit-llvm`: 走完代码生成阶段，输出 LLVM IR 文本（`.ll`）。
...
#### 编译管线执行流 (`run()` 方法，clang 风格 Driver/Compilation/Command)

```mermaid
graph TD
    Start[解析命令行参数] --> Init[初始化基础组件 (SM, Diag)]
    Init --> Frontend[运行 FrontendCommand: Lexer -> Parser -> Sema]
    Frontend -.->|出错或仅检查模式| End
    Frontend --> CodeGen[运行 FrontendAction: CodeGen -> LLVM IR/Object]
...
3. 在 `src/Driver/Driver.cpp` 的命令构建与执行阶段（`Compilation/Command`）插入你的模块调用，并妥善处理它的错误返回。
