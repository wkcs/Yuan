...
本文档只描述当前版本已经承诺的类型语义和兼容性规则，不描述尚未稳定的未来设计。
...
5. **显式引用优先**:
   - 普通函数调用、赋值和返回以显式写法为准；需要引用时，应显式写出 `&obj` 或 `&mut obj`。
   - 方法调用路径中仍可能存在有限的实现级接收者归一化，但不应作为语言承诺依赖。
   - 当前版本不承诺 Rust 风格的通用 auto-borrow / auto-deref 体验。
6. **整数自动字面量适配**:
...
- 当前版本只对 `move` 之后的使用做静态检查，例如 `use-after-move` 和 `maybe-moved`。
- 当前版本不保证发现全部别名写冲突。
- 当前版本不保证发现全部悬垂引用。
- `&mut T` 只表达可变访问权限，不表达并发安全。
